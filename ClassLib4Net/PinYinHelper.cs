...
namespace ClassLib4Net
{
	...
	public class PinYinHelper
	{
		...
		/// 匹配单个中文字符
		/// </summary>
		public static readonly System.Text.RegularExpressions.Regex regexSingleChineseChar = new System.Text.RegularExpressions.Regex("^[\u4e00-\u9fa5]$");
		/// <summary>
		/// 匹配单个英文字符
		/// </summary>
		public static readonly System.Text.RegularExpressions.Regex regexSingleEnglishChar = new System.Text.RegularExpressions.Regex("^[a-zA-Z]$");

		#region 获取汉字字符串转拼音的首字母字符串 II
		...
		public static string GetSpellCode(string ChineseText)
		{
			string strRet = string.Empty;
			char[] ArrChar = ChineseText.ToCharArray();
			foreach (char c in ArrChar)
			{
				strRet += GetSpellCodeBySingleChineseChar(c.ToString());
			}
			return strRet;
		}
		/// <summary>
		...
		public static string GetFirstSpellCode(string ChineseText)
		{
			string strRet = string.Empty;
			char[] ArrChar = ChineseText.ToCharArray();
			foreach (char c in ArrChar)
			{
				strRet += GetFirstSpellCodeBySingleChineseChar2(c.ToString());
			}
			return strRet;
		}
		...
		/// <returns>拼音码字符串（全拼）</returns> 
		private static string GetSpellCodeBySingleChineseChar(string SingleChineseChar)
		{
			if (regexSingleChineseChar.IsMatch(SingleChineseChar))
			{
				byte[] array;
				int iAsc;
				string strRtn = string.Empty;

				array = Encoding.Default.GetBytes(SingleChineseChar);
				try
				{
				...
				if (!string.IsNullOrWhiteSpace(strRtn) && strRtn != "Zuo")
				{
					//将首字母转为大写
					if (strRtn.Length > 1)
					{
				...
				{
					strRtn = GetPY(SingleChineseChar);
				}

				return string.IsNullOrWhiteSpace(strRtn) ? SingleChineseChar : strRtn; //如果不是汉字则返回
			}
			else
			{
				string py = GetPY(SingleChineseChar);
				return string.IsNullOrWhiteSpace(py) ? SingleChineseChar : py; //如果不是汉字则返回
			}
		}
		...
		/// </summary> 
		/// <returns>首字母</returns> 
		private static string GetFirstSpellCodeBySingleChineseChar2(string SingleChineseChar)
		{
			return GetSpellCodeBySingleChineseChar(SingleChineseChar).Substring(0, 1);
		}
		#endregion
		...
		#region 超级全
		private static readonly string[,] ChineseCharArray = new string[,]{
			{"A","啊阿呵吖嗄腌锕錒"},
		...
		/// <param name="SingleChineseChar">单个汉字</param>
		/// <returns></returns>
		public static string GetPY(string SingleChineseChar)
		{
			if (!string.IsNullOrWhiteSpace(SingleChineseChar))
			{
				if (regexSingleChineseChar.IsMatch(SingleChineseChar))
				{
					for (int i = 0; i < ChineseCharArray.Length; i++)
					{
						if (ChineseCharArray[i, 1].IndexOf(SingleChineseChar) > -1)
							return ChineseCharArray[i, 0];
					}
				}
				else if (regexSingleEnglishChar.IsMatch(SingleChineseChar))
				{
					return SingleChineseChar;
				}
			}
